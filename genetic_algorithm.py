...
def init_individual_(num_teams, fixed_groups, players):
    individual = [random.randint(0, num_teams - 1) for _ in range(len(players))]
    for group in fixed_groups:
        team = random.randint(0, num_teams - 1)
...
def custom_mutate_(individual, indpb, fixed_groups, players, num_teams):
    for group in fixed_groups:
        if random.random() < indpb:
            new_team = random.randint(0, num_teams - 1)
            for player_name in group:
                for i, player in enumerate(players):
                    if player['name'] == player_name:
                        individual[i] = new_team
    return individual,

...
def swap_members(data, swap_info):
    ...
    for member1, member2 in swap_pairs:
        ...
        # Update total score with rounding to 1 decimal place after each calculation
        data['results'][team1]['Total Score'] = round(data['results'][team1]['Total Score'], 1)
        data['results'][team2]['Total Score'] = round(data['results'][team2]['Total Score'], 1)
...
def main():
    ...
    updated_filepath = save_json(args.filepath, updated_data)
    json_to_png(updated_filepath)
